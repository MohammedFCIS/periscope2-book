...
Hence, module input will be any data set (data.frame in `R`) and the output will be up-to-certain limit an interactive report that displays:

-   Data summary (number of features and number of observations)
-   Features type, properties and statistical summary
-   Features visualization

Check @fig-2_1 for module objective summary. 

```{mermaid}
%%| fig-cap: "Feature Analyzer Module (FAM) Objective"
%%| fig-width: 8 
%%| label: fig-2_1
flowchart LR
...
Rest of this chapter will discuss module input, output and functions in more details while building it step by step

## Module Input
Module will have two different input types:

- Data input: as stated in @fig-2_1, that will be any `R` data.frame object
...
As data input is the main input, the next sections will discuss what is data in the first place? what are data features? and what is the difference between the data features and data observations? and what are the available features types and how to measure them statistically?
...
The simplest method to visualize categorical data. It is a circle divided into sectors. Each sector represent a feature relative frequency (upon hovering over plot, both frequency and relative frequency will be shown). Sum of the sectors equal 100%, review @fig-2_6
...
Another method to visualize categorical data. Usually, each category is represented as a bar in the X-axis while bar height represents either category frequency (absolute or relative). The tallest bar is representing the *mode*, review @fig-2_7
...
![Absolute Frequencies](Images/2_3_a_bar_plot.png){#fig-2_6a width=48%}
![Relative Frequencies](Images/2_3_b_bar_plot.png){#fig-2_6b width=48%}
![Absolute Stacked Frequencies](Images/2_3_c_bar_plot.png){#fig-2_6c width=48%}
![Relative Stacked Frequencies](Images/2_3_d_bar_plot.png){#fig-2_6d width=48%}
...
However, bar plot can be customized to view data in different ways. For example, instead of displaying frequencies on the Y-axis, relative frequencies can be displayed instead.  Also, bars cab via stacked above each other to show cumulative distribution along with regular distribution 

:::{.callout-tip}
If categories count is large, bar plot became so dense and might be hard to digest
:::
...
The following are histogram examples (check Ch2_2.4.R for generating the below charts) ![Histogram](Images/2_4_a_hist_plot.png) ![Histogram Dot plot](Images/2_4_b_hist_plot.png)
...
The following are density plot examples (check Ch2_2.5.R for generating the below charts) ![Histogram](Images/2_5_a_densityt_plot.png) ![Histogram](Images/2_5_b_densityt_plot.png)

#### Boxplot

One of the best methods to describe numerical feature measure tendency and dispersion is **Boxplot**. It shows median, quartiles, minimum, maximum, and outlier values in the same plot easily, these info can be displayed via plot tooltop upon hovering on the box.

The following are histogram examples (check Ch2_2.6.R for generating the below charts) ![Boxplot](Images/2_6_a_box_plot.png) ![Boxplot tooltip](Images/2_6_b_box_plot.png)
