...
*Note: Correlation among features will be discussed in more details in next chapter.*
...
Check Figure 2.1 for module objective summary. 

```{mermaid}
%%| fig-cap: "Figure 2.1: Feature Analyzer Module (FAM) Objective"
%%| fig-width: 8 
flowchart LR
  A[R data.frame] --> B>"Feature Analyzer Module
...
But first what is data in first place? what are data features? and what is the difference between the data features and data observations? and what are the available features types and how to measure them statistically? and how to use `R` to measure them?

In the following sections we will seek the answer for these questions and more
...
TODO: show available datasets in R and briefly how to load external datasets in R and build downloadable table shows available datasets name and their dimensions
...
![Frequency Barplot](Images/2_2_a_pie_chart.png) ![Frequency Barplot](Images/2_2_b_pie_chart.png) (check Ch2_2.2.R for generating the above charts)
...
However, bar plot can be customized to view data in different ways. For example, instead of displaying frequencies on the Y-axis, relative frequencies can be displayed instead ![Relative Frequency Barplot](Images/2_3_b_bar_plot.png) Also, bars cab via stacked above each other to show cumulative distribution along with regular distribution ![Frequency Stacked Barplot](Images/2_3_c_bar_plot.png)
